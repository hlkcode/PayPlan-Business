...
const onBulkActivate = async (activate: boolean) => {
    ...
    for (const id of selectedIds.value) {
        // We don't have the full row here easily without looking it up from dataTable data
         // But for activation we only need ID.
         // However, ideally we check current status to avoid redundant calls, but the API might handle it.
         // Let's just call it.
         try {
    ...
    if (successCount > 0) {
        tableRef.value?.refresh()
        selectedIds.value = [] // Clear selection? Or keep it? Usually keep.
        toast.add({
            type: 'success',
